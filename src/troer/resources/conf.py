...
root_doc = 'index'
project = os.getenv('PROJECT', u'??')
master_title = project.title()
author = u'Troer'
copyright = u"LGPL-3.0-only"
...
latex_documents = [
    ('index', project + '.tex', master_title, author, 'manual')
]
...
dpackdoc_target_path = os.getenv('DPACKDOC_TARGET_PATH')
if not dpackdoc_target_path:
    dpackdoc_target_path = '../../dpack/html'

dpackdoc_inventory_path = os.getenv('DPACKDOC_INVENTORY_PATH')
if not dpackdoc_inventory_path:
    docdir = os.getenv('DOCDIR')
    if docdir:
        dpackdoc_inventory_path = os.path.join(docdir,
                                               '../dpack/html/objects.inv')
...
intersphinx_mapping = {
        ...
        'dpack':  (dpackdoc_target_path,  dpackdoc_inventory_path),
